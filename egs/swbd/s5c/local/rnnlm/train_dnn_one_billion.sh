...
text_dir=data/dnn_with_lm1b/text
one_billion_train=data/rnnlm/lm1b/lm1b.txt
one_billion_dev=data/rnnlm/lm1b/dev_lm1b.all.txt
conv_lines=1000
mkdir -p $dir/config
set -e

for f in $text $one_billion_train $one_billion_dev; do
  [ ! -f $f ] && \
...
if [ $stage -le 0 ]; then
  ...
  # prepare one-billion data
  n=`cat $one_billion_train | wc -l`
  num_lines=`echo $(( $[n/conv_lines] * $conv_lines ))`
  head -n $num_lines $one_billion_train > $text_dir/lm1b.txt
  head -n 100000 $text_dir/lm1b.txt > $text_dir/lm1b_sub.txt
  head -n 100000 $one_billion_dev > $text_dir/dev_lm1b.txt
...
EOF
  ...
  rnnlm/get_conv_egs_with_lm1b.py --vocab-file=$dir/config/words.txt \
                        --unk-word="<unk>" \
  ...
  cat $dir/egs/swbd_sub.txt $dir/egs/fisher_sub.txt $dir/egs/lm1b_sub.txt > $dir/egs/train_subset.txt
  cat $dir/egs/swbd_sub.label.txt $dir/egs/fisher_sub.label.txt $dir/egs/lm1b_sub.label.txt > $dir/egs/train_subset.label.txt

  cat $dir/egs/dev_swbd.txt $dir/egs/dev_fisher.txt $dir/egs/dev_lm1b.txt > $dir/egs/dev.txt
  cat $dir/egs/dev_swbd.label.txt $dir/egs/dev_fisher.label.txt $dir/egs/dev_lm1b.label.txt > $dir/egs/dev.label.txt

  cat $dir/egs/swbd.txt $dir/egs/fisher.txt $dir/egs/lm1b.txt > $dir/egs/train.txt
  cat $dir/egs/swbd.label.txt $dir/egs/fisher.label.txt $dir/egs/lm1b.label.txt > $dir/egs/train.label.txt
...
relu-renorm-layer name=tdnn3 dim=$embedding_dim input=Append(0)
