...
parser.add_argument("--conv-lines", type=int, default=500)
...
def get_conv_counts(data_sources, data_weights, vocab, lines):
    ...
    lm1b = defaultdict(lambda:defaultdict(lambda:defaultdict()))
    lm1b_sub = defaultdict(lambda:defaultdict(lambda:defaultdict()))
    dev_lm1b = defaultdict(lambda:defaultdict(lambda:defaultdict()))
    ...
    for name, (text_file, _) in data_sources.items():
        ...
        if name == "lm1b":
            lm1b = get_conv_lm1b(text_file, weight, vocab, lines)
        if name == "lm1b_sub":
            lm1b_sub = get_conv_lm1b(text_file, weight, vocab, lines)
        if name == "dev_lm1b":
            dev_lm1b = get_conv_lm1b(text_file, weight, vocab, lines)

    return fisher, fisher_sub, dev_fisher, swbd, swbd_sub, dev_swbd, lm1b, lm1b_sub, dev_lm1b

def get_conv_lm1b(filename, weight, vocab, lines):
    counts = defaultdict(lambda:defaultdict(lambda:defaultdict()))
    total_counts = defaultdict(lambda:defaultdict())
    line_num = 0 
    with open(filename, 'r', encoding="utf-8") as f:
        for line in f:
            key = str(int(line_num / lines)) 
            line_num += 1
            fields = line.split()
            if len(fields) < 2:
                continue
            
            id_side = 'a' if line_num % lines < lines / 2 else 'b'
            # print(id_side, line_num % lines)
            for word in fields:
                if word not in vocab:
                    if args.unk_word == '':
                        ...
                        word = args.unk_word
                if word in counts[key][id_side]:
                    counts[key][id_side][word] += weight
                else:
                    counts[key][id_side][word] = weight
                if id_side == 'a':
                    if id_side in total_counts[key]:
                        total_counts[key][id_side] += 1
                    else:
                        total_counts[key][id_side] = 1

    # normalize training data with data_weight
    for key, conv_map in counts.items():
        for id_side, word_counts in conv_map.items():
            if id_side == 'a':
                for word, count in word_counts.items():
                    counts[key][id_side][word] = count * 1.0 / total_counts[key][id_side]
    return counts
...
fisher_input, fisher_sub_input, dev_fisher_input, swbd_input, swbd_sub_input, dev_swbd_input, lm1b_input, lm1b_sub_input, dev_lm1b_input = get_conv_counts(data_sources, data_weights, vocab, args.conv_lines)
